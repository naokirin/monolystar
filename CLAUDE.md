...
また同タブの抽出条件は、未完了のワンショットタスクは締切（`endDate`）超過後も表示し続ける独自解釈
（`todaySort.ts` の `isOneShotTaskForToday`。仕様書4.5に追記済み）。定期タスクの有効期間判定
（`recurrence.ts`）はこの対象外で従来通り。

締切の近さ段階（上記3.）はさらに「近い／締切未設定／遠い」の3バケットに分かれ、締切までの残り日数が
`Prefs.farDeadlineThresholdDays`（既定7日）以上のタスクは締切未設定より下位に表示する
（`todaySort.ts` の `deadlineBucket`）。この閾値はヘッダーの⚙️から開く設定画面
（`src/lib/components/SettingsMenu.svelte`）でユーザーが変更できる（仕様書4.1.1・4.5）。

タスクカードの締切警告色表示（仕様書4.7）は、締切24時間以内に加えて締切超過（既に過ぎている）も
対象に含める独自解釈（`src/lib/logic/dates.ts` の `isDeadlineUrgent`。`TaskCard.svelte` から利用）。
